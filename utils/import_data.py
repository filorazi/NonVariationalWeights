...
def get_dataset(dataset):
    if dataset == "MNIST09":
        ...
        X,Y = load_nldummy()
        X_train, X_test, Y_train, Y_test = train_test_split(X, Y, random_state=111, test_size=0.2)
    elif dataset == "linear_dummy":
        X,Y = load_ldummy()
        X_train, X_test, Y_train, Y_test = train_test_split(X, Y, random_state=111, test_size=0.2)
    elif dataset == "sigmoid":
        X,Y = load_sigmoid()
        X_train, X_test, Y_train, Y_test = train_test_split(X, Y, random_state=111, test_size=0.2)
    else:
...
def load_sigmoid(n=100,rand_noise=0.03):
    x = np.linspace(0.01,1,n)
    def sigmoid(z):
        return 1/(1 + np.exp(-z))
    return x.reshape(n, 1), sigmoid(x*12-6)

def load_ldummy(n=200,rand_noise=0.03):
    ...
    for el in x:
        for il in y:
            l.append([el,il,(el+il)*.5+random.uniform(-rand_noise, rand_noise)])
    ds = pd.DataFrame(l, columns = ["x1", "x2","y"] )
    Y = ds['y']
    X = ds.drop(["y"], axis=1) 
    X,Y = resample(X,Y, n_samples=n)
    Y = Y.to_numpy()
    # removed normalization since x and y are built to be in [0,1]
    #y_raw = (Y - np.min(Y)) / (np.max(Y) - np.min(Y))
    
    X = X.to_numpy()

    # Rescaleing of the values of the features
    #x_raw = (X - np.min(X)) / (np.max(X) - np.min(X))
    
    return X, Y#x_raw, y_raw


def load_nldummy(n =200):
    from sklearn.utils import resample
    ...
    for el in x:
        for il in y:
            l.append([el,il,(np.sin((el+il)*2*np.pi))*.5+.5])
    ds = pd.DataFrame(l, columns = ["x1", "x2","y"] )
    Y = ds['y']
    X = ds.drop(["y"], axis=1) 
    X,Y = resample(X,Y, n_samples=n)
...
def load_diabetes_p(n =200):
    ...
    pca_df = pd.DataFrame(np.vstack((principalComponents.T, Y)).T,
                          columns=['x1', 'x2', 'Y'])
